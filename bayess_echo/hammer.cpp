...
void print_array(double*** confMat, int len){
	cout<< "printMatrix\n";
	for (int i = 0; i < len;++i){
		for (int j = 0; j < 4; ++j){
			for (int k = 0; k < 4; ++k){
...
double logLikelihood(Clusters& clusters, double*** confMat) {
	...
	//print_array(confMat, clusters.centers[0].length());
...
bool is_in_group(vector<string>& centroids, string& new_center){
	for (int j = 0; j < centroids.size(); ++j){
		if (new_center == centroids[j]){
			return true;
		}
	}
	return false;
...
Clusters* kmeans(int k, vector<Brother>& reads, double*** confMatrix){
	...
	int false_iterations = 0;
	bool previous_false = false;
	vector<string> prev_centroids = centroids;
	vector<vector<Brother> >  groups;
	bool first_pass = true;
	int iteration = 1;
	while ((first_pass || !vector_equals(centroids, prev_centroids)) && iteration < 100){
	//	cerr<<"ITERATION "<< iteration << "\n";
		...
		for (size_t i = 0; i < groups.size(); ++i){
			if (groups[i].size() == 0){
				if (previous_false == true)
			 		return new Clusters(vector<string>(), groups);
			 	else {
			 		previous_false = true;
			 		centroids = kpp(k, reads, confMatrix);
			 	}
			}
		}
		if (previous_false)
		...
		for (size_t i = 0; i < groups.size(); ++i){
			string new_center = find_centroid(groups[i], confMatrix);
...
//	cerr << "begin hammer\n";
	...
	Clusters* clusters;
	do{	
//		cerr << "number of clusters "<< num_clust <<"\n";
		prevLog = log;
		clusters = kmeans(num_clust, brothers, confMat);
		log = logLikelihood(*clusters, confMat);
		num_clust++;
//		cout << log << "\n";
	} while(prevLog < log && clusters->centers.size() == num_clust -1);
