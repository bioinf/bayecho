...
int main(int argc, char** argv) {
    ...
    std::cout << "Parallel Neighboring (no param)" << std::endl;
    ...
    std::cout << "\t max_cov: " << opt.max_cov << std::endl;
        ...
        for (int kmer_index = 0; kmer_index < nKmer; kmer_index++) {
            ...
            for(vector<KmerOccurrence>::iterator read1=kmer_occurrences.begin(); read1!=kmer_occurrences.end(); ++read1) {
                ...
                for (vector<KmerOccurrence>::iterator read2 = kmer_occurrences2.begin(); read2 != kmer_occurrences2.end(); ++read2) {
                    ...
                    neighbors_map::const_accessor ac_neighbor2;
                    if (neighbors.find(ac_neighbor2, readid2)) {
                        if (ac_neighbor2->second.size() > NeighborSet::MaxNNeighbors) continue;
                    } else {
                        neighbors.insert(ac_neighbor2, readid2);
                    } 
                    ...
                    if ((!seen && is_neighbor) || (seen && is_better)) {
                        ...
                        neighbors.find(w_ac_neighbor2, readid2);
